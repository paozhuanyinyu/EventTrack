...
public class EventTrackHelper {
    ...
    public static void trackViewOnClick(DialogInterface dialogInterface, int whichButton) {
        try {
            ...
            //将Context转成Activity
            Activity activity = EventTrackPrivate.getActivityFromContext(context);
            ...
            EventTrackAPI.getInstance().track("$AppClick", properties);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void trackViewOnClick(Object object, MenuItem menuItem) {
        try {
            ...
            EventTrackAPI.getInstance().track("$AppClick", jsonObject);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void trackExpandableListViewChildOnClick(ExpandableListView expandableListView, View view,
        ...
        try {
            ...
            String idString = EventTrackPrivate.getViewId(expandableListView);
            if (!TextUtils.isEmpty(idString)) {
                properties.put("$element_id", idString);
            }
            ...
            if (view instanceof ViewGroup) {
                try {
                    ...
                    viewText = EventTrackPrivate.traverseViewContent(stringBuilder, (ViewGroup) view);
                    if (!TextUtils.isEmpty(viewText)) {
    ...
    public static void trackViewOnClick(android.widget.AdapterView adapterView, android.view.View view, int position) {
        try {
            ...
            Activity activity = EventTrackPrivate.getActivityFromContext(context);
            String idString = EventTrackPrivate.getViewId(adapterView);
            if (!TextUtils.isEmpty(idString)) {
                properties.put("$element_id", idString);
            }
            ...
            if (adapterView instanceof Spinner) {
                ...
                if (view instanceof ViewGroup) {
                    ...
                    viewText = EventTrackPrivate.getElementContent(view);
                }
                //$element_content
                if (!TextUtils.isEmpty(viewText)) {
                    properties.put("$element_content", viewText);
    ...
    public static void trackViewOnClick(View view) {
        try {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("$element_type", EventTrackPrivate.getElementType(view));
            jsonObject.put("$element_id", EventTrackPrivate.getViewId(view));
            jsonObject.put("$element_content", EventTrackPrivate.getElementContent(view));

            Activity activity = EventTrackPrivate.getActivityFromView(view);
            if (activity != null) {
                jsonObject.put("$activity", activity.getClass().getCanonicalName());
            }
